...
def updateScore(tname, addPts, solved):
	...
	query = "CREATE OR REPLACE VIEW user_scores AS SELECT * FROM score ORDER BY (score) DESC, timestmp ASC"
	cursor.execute(query)
		...
		if solved.find(prob) >= 0:
			...
			if ret != 0 and j.language != 'python':
				...
				while True:
					...
					# Break if subprocess has terminated or runtime has exceeded 5.0s
					if ((p.returncode != None) or (runTime > 5.00001)):
						break;
